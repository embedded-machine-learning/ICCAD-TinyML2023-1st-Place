...
def test_ConvBn():
    DUT = ConvBn(in_channels,out_channels,3,momentum=1,out_quant_kargs={'mom1':1})
    ...
    for i in range(100):
        ...
        mask = torch.isclose(out_val,should_be,atol=1.1*float(DUT.bn.out_quant.delta_out.max().numpy()))
